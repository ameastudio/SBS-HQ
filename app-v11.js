...
async function restoreSession(){
  try{
    ...
    console.error("SBS HQ session restore failed:", error);
...
async function safeQ(label,table,select="*",opts={}){
  try{
    return {ok:true,label,data:await q(table,select,opts),error:null};
  }catch(error){
    console.error(`SBS HQ load failed for ${label}:`,error);
    return {ok:false,label,data:[],error};
  }
}

async function refreshAll(){
  const results=await Promise.all([
    safeQ("profiles","profiles","id,full_name,email,phone,instagram_handle,role,created_at",{order:"created_at",ascending:false}),
    safeQ("manual clients","manual_clients","*",{order:"created_at",ascending:false}),
    safeQ("shoot types","shoot_types","*",{order:"sort_order"}),
    safeQ("packages","shoot_packages","*",{order:"sort_order"}),
    safeQ("bookings","bookings","*",{order:"created_at",ascending:false}),
    safeQ("payments","payments","*",{order:"created_at",ascending:false}),
    safeQ("invoices","invoices","*",{order:"created_at",ascending:false}),
    safeQ("extras","booking_adjustments","*",{order:"created_at"}),
    safeQ("inspiration photos","booking_inspiration","*",{order:"sort_order"}),
    safeQ("blocked slots","blocked_slots","*",{order:"block_date"}),
    safeQ("portfolio","portfolio_items","*",{order:"sort_order"}),
    safeQ("reviews","reviews","*",{order:"created_at",ascending:false}),
    safeQ("website content","site_content","key,value")
  ]);

  const [
    profilesR,manualClientsR,shootTypesR,packagesR,bookingsR,paymentsR,invoicesR,
    adjustmentsR,inspirationsR,blockedSlotsR,portfolioR,reviewsR,siteRowsR
  ]=results;

  Object.assign(state,{
    profiles:profilesR.data,
    manualClients:manualClientsR.data,
    shootTypes:shootTypesR.data,
    ...
    inspirations:inspirationsR.data,
    blockedSlots:blockedSlotsR.data,
    portfolio:portfolioR.data,
    reviews:reviewsR.data
  });

  state.siteContent=Object.fromEntries(siteRowsR.data.map(r=>[r.key,r.value||{}]));
  renderAll();

  const failed=results.filter(r=>!r.ok);
  const requiredFailed=failed.filter(r=>["profiles","bookings"].includes(r.label));

  if(requiredFailed.length){
    const details=requiredFailed
      .map(r=>`${r.label}: ${r.error?.message||"unknown error"}`)
      .join("\n");
    alert(`SBS HQ loaded, but the main studio data could not sync yet.\n\n${details}`);
  }else if(failed.length){
    console.warn("SBS HQ optional sections still syncing:",failed.map(r=>r.label));
  }
}
function renderAll(){
...
function bookingCard(b,compact=false){
  return `<article class="booking-card">
    ...
    ${b.client_request_type ? `
      <p class="info-note">
        <strong>CLIENT ${b.client_request_type==="reschedule"?"RESCHEDULE":"CANCELLATION"} REQUEST:</strong>
        ${
          b.client_request_type==="reschedule"
            ? `${escapeHtml(dateText(b.client_requested_date))} at ${escapeHtml(timeText(b.client_requested_time))}`
            : "Client wants to cancel this booking."
        }
        ${b.client_request_note ? ` — ${escapeHtml(b.client_request_note)}` : ""}
      </p>
    ` : ""}
...
function renderDashboard(){
  ...
  const clientChanges=state.bookings
    .filter(b=>b.client_request_type)
    .sort((a,b)=>String(b.client_request_created_at||"").localeCompare(String(a.client_request_created_at||"")));
  ...
  $("#homeClientChanges").innerHTML=clientChanges.length?clientChanges.map(b=>bookingCard(b,true)).join(""):emptyCard("No reschedule or cancellation requests.");
...
async function deleteBookingPermanently(id){
  const b=state.bookings.find(x=>x.id===id);
  if(!b)return;

  const c=getClientForBooking(b);
  const who=c?.full_name||c?.email||"this client";
  const ok=confirm(`Permanently delete this booking for ${who}?\n\nThis removes the booking, linked payments, invoices, inspiration records and extra charges. The client account will NOT be deleted.`);
  if(!ok)return;

  const paymentPaths=state.payments
    .filter(p=>p.booking_id===id && p.proof_url)
    .map(p=>p.proof_url);
  const inspirationPaths=state.inspirations
    .filter(x=>x.booking_id===id && x.image_url)
    .map(x=>x.image_url);

  const {error}=await sb.rpc("admin_delete_booking",{p_booking_id:id});
  if(error){
    alert(error.message);
    return;
  }

  // Storage cleanup is best-effort. Database deletion has already succeeded.
  if(paymentPaths.length){
    const {error:storageError}=await sb.storage.from("payment-proofs").remove(paymentPaths);
    if(storageError)console.warn("Could not remove one or more payment-proof files:",storageError);
  }
  if(inspirationPaths.length){
    const {error:storageError}=await sb.storage.from("booking-inspiration").remove(inspirationPaths);
    if(storageError)console.warn("Could not remove one or more inspiration files:",storageError);
  }
...
async function resolveClientRequest(id, action){
  const b=state.bookings.find(x=>x.id===id);
  if(!b || !b.client_request_type)return;
  ...
  const clearFields={
    ...
    client_request_created_at:null
  };

  if(action==="approve"){
    if(b.client_request_type==="reschedule"){
      if(!b.client_requested_date || !b.client_requested_time){
        return alert("This reschedule request is missing a date or time.");
      }

      if(!confirm(`Approve reschedule to ${dateText(b.client_requested_date)} at ${timeText(b.client_requested_time)}?`))return;

      const {error}=await sb.from("bookings").update({
        preferred_date:b.client_requested_date,
        preferred_time:b.client_requested_time,
        reschedule_count:Number(b.reschedule_count||0)+1,
        ...clearFields
      }).eq("id",id);
      ...
      await refreshAll();
      closeModals();
      return alert("Reschedule approved.");
    }

    if(b.client_request_type==="cancel"){
      ...
      await refreshAll();
      closeModals();
      return alert("Cancellation approved.");
    }
  }

  if(action==="decline"){
    if(!confirm("Decline this client request?"))return;

    const {error}=await sb.from("bookings").update(clearFields).eq("id",id);
    if(error)return alert(error.message);
...
async function openBookingModal(id){
  ...
  const clientRequest = b.client_request_type ? `
    <section class="modal-section request-alert">
      ...
      <div class="card-actions">
        <button class="action-primary" data-client-request-action="approve" data-id="${b.id}">
          ${b.client_request_type==="reschedule" ? "APPROVE RESCHEDULE" : "APPROVE CANCELLATION"}
        </button>
        <button class="action-secondary" data-client-request-action="decline" data-id="${b.id}">
          DECLINE REQUEST
        </button>
      </div>
    </section>
  ...
  $("#bookingModalContent").innerHTML=`
    ...
    <section class="modal-section">
      <h4>Danger Zone</h4>
      <p class="info-note">Permanently deletes this booking and its linked payment, invoice, inspiration and extra-charge records. The client account stays.</p>
      <button class="action-danger full" type="button" data-delete-booking="${b.id}">DELETE BOOKING PERMANENTLY</button>
    </section>
  `;
...
function bindBookingModalForms(b){
  ...
  $("#downloadLinkForm")?.addEventListener("submit",async e=>{
    ...
    const originalText=button?.textContent||"SAVE PHOTO LINK";
    if(button){
    ...
    const {data,error}=await sb.rpc("admin_set_booking_download_url",{
      p_booking_id:b.id,
      p_download_url:url||null
    });
    ...
    const saved=state.bookings.find(x=>x.id===b.id);
    if((saved?.download_url||"") !== url){
      alert("The photo link did not save. Please try again.");
      return;
    }

    alert("Photo link saved.");
    closeModals();
  });
}
...
function packageSummary(p){
  const bits=[];
  if(p.duration_minutes)bits.push(`${Number(p.duration_minutes)} min`);
  if(p.photos_received!=null&&p.photos_received!=="")bits.push(`${Number(p.photos_received)} photos`);
  if(p.edited_photos!=null&&p.edited_photos!=="")bits.push(`${Number(p.edited_photos)} edited`);
  return bits.join(" • ");
}
  ...
  $("#shootTypesList").innerHTML=state.shootTypes.length?state.shootTypes.map(t=>{
    ...
    return `<article class="type-card">
      <div class="type-card-top">
        <div class="type-photo-control">
          <div class="type-thumb ${img?"has-photo":"no-photo"}">${img?`<img src="${escapeHtml(img)}" alt="${escapeHtml(t.name)} photo">`:`<span>PHOTO</span>`}</div>
          <label class="quick-photo-button">${img?"CHANGE PHOTO":"ADD PHOTO"}<input type="file" accept="image/*" data-quick-type-photo="${t.id}" hidden></label>
        </div>
        <div class="type-card-copy"><h4>${escapeHtml(t.name)}</h4><p>${escapeHtml(t.description||"No description yet.")}</p><div class="inline-actions"><button class="action-secondary" data-edit-type="${t.id}">EDIT DETAILS</button><button class="action-primary" data-add-package="${t.id}">+ ADD OPTION</button></div></div>
      </div>
      <div class="type-options-head"><span>OPTIONS CLIENTS SEE</span><strong>${packs.length}</strong></div>
      <div class="package-mini-list">${packs.length?packs.map(p=>`<div class="package-mini"><div class="package-mini-copy"><strong>${escapeHtml(p.name)}</strong><span class="package-price">${escapeHtml(p.price_label||money(p.price))}</span>${packageSummary(p)?`<small>${escapeHtml(packageSummary(p))}</small>`:""}</div><button data-edit-package="${p.id}">EDIT</button></div>`).join(""):`<div class="empty-option"><span>No options added yet.</span><button type="button" data-add-package="${t.id}">+ ADD FIRST OPTION</button></div>`}</div>
    </article>`;
...
function openShootTypeEditor(id){
  ...
  $("#shootTypeForm").innerHTML=`
    <div class="shoot-photo-editor">
      <div class="shoot-photo-editor-head"><div><span class="mini-label">SHOOT PHOTO</span><strong>${imgs.length?"Current gallery":"No photo added yet"}</strong></div><label class="photo-picker-button">+ ADD PHOTOS<input id="typeImages" type="file" accept="image/*" multiple hidden></label></div>
      <p class="upload-help">Add up to 3 high-quality photos. Large images are automatically optimized for the website.</p>
      <div class="image-strip shoot-gallery-strip">${imgs.length?imgs.map((u,i)=>`<div class="shoot-gallery-item"><img src="${escapeHtml(u)}" alt=""><button class="image-remove-button" type="button" data-remove-type-image="${i}">×</button></div>`).join(""):`<div class="shoot-photo-placeholder">Choose a photo so clients can quickly recognize this shoot type.</div>`}</div>
    </div>
...
const IMAGE_OPTIMIZE_AT_BYTES=16*1024*1024;
const IMAGE_MAX_DIMENSION=5000;
function imageExtension(name){const m=String(name||"").toLowerCase().match(/\.(jpe?g|png|webp)$/);return m?m[1]:"jpg"}
async function imageDimensions(file){
  return await new Promise((resolve,reject)=>{const img=new Image(),url=URL.createObjectURL(file);img.onload=()=>{const out={width:img.naturalWidth||img.width,height:img.naturalHeight||img.height,img};URL.revokeObjectURL(url);resolve(out)};img.onerror=()=>{URL.revokeObjectURL(url);reject(new Error("This image could not be opened. Please choose another photo."))};img.src=url});
}
async function optimizeImageForUpload(file){
  if(!file||!String(file.type||"").startsWith("image/"))return file;
  if(file.size>MAX_IMAGE_UPLOAD_BYTES)throw new Error("That photo is over 50 MB. Please choose a photo under 50 MB.");
  let dims;try{dims=await imageDimensions(file)}catch{return file}
  const needsResize=Math.max(dims.width,dims.height)>IMAGE_MAX_DIMENSION;
  const needsCompress=file.size>IMAGE_OPTIMIZE_AT_BYTES;
  if(!needsResize&&!needsCompress)return file;
  const scale=Math.min(1,IMAGE_MAX_DIMENSION/Math.max(dims.width,dims.height));
  const width=Math.max(1,Math.round(dims.width*scale)),height=Math.max(1,Math.round(dims.height*scale));
  const canvas=document.createElement("canvas");canvas.width=width;canvas.height=height;
  const ctx=canvas.getContext("2d",{alpha:false});ctx.imageSmoothingEnabled=true;ctx.imageSmoothingQuality="high";ctx.drawImage(dims.img,0,0,width,height);
  let quality=.94,blob=null;
  while(quality>=.80){blob=await new Promise(r=>canvas.toBlob(r,"image/jpeg",quality));if(blob&&blob.size<=IMAGE_OPTIMIZE_AT_BYTES)break;quality-=.04}
  if(!blob)return file;
  const base=String(file.name||"photo").replace(/\.[^.]+$/,"").replace(/[^a-zA-Z0-9._-]/g,"-");
  return new File([blob],`${base}-web.jpg`,{type:"image/jpeg",lastModified:Date.now()});
}
async function uploadPublic(file,folder){
  const prepared=await optimizeImageForUpload(file);
  const safe=prepared.name.replace(/[^a-zA-Z0-9._-]/g,"-"),path=`${folder}/${Date.now()}-${safe}`;
  const {error}=await sb.storage.from("site-media").upload(path,prepared,{upsert:false,contentType:prepared.type||undefined});
  if(error)throw error;
  return sb.storage.from("site-media").getPublicUrl(path).data.publicUrl;
}
async function quickUpdateShootPhoto(input){
  const id=input?.dataset?.quickTypePhoto,file=input?.files?.[0],t=getType(id);if(!id||!file||!t)return;
  const label=input.closest("label"),original=label?.childNodes?.[0]?.textContent||"CHANGE PHOTO";
  if(label){label.classList.add("is-uploading");label.childNodes[0].textContent="UPLOADING…"}
  let newUrl="";
  try{
    newUrl=await uploadPublic(file,`shoot-types/${id}`);
    const old=typeGallery(t);const oldFirst=old[0]||t.image_url||"";
    const imgs=[newUrl,...old.slice(1)].slice(0,3);
    const {error}=await sb.from("shoot_types").update({gallery_images:imgs,image_url:newUrl}).eq("id",id);if(error)throw error;
    if(oldFirst&&oldFirst!==newUrl)await removeSiteMediaUrl(oldFirst);
    await refreshAll();
  }catch(error){if(newUrl)await removeSiteMediaUrl(newUrl);alert(error.message||"The photo could not be saved.")}
  finally{if(label){label.classList.remove("is-uploading");if(label.childNodes[0])label.childNodes[0].textContent=original}}
}
...
function openPackageEditor(id,typeId){
  ...
  const sid=typeId||p?.shoot_type_id;$("#packageModalTitle").textContent=p?"Edit Shoot Option":"Add Shoot Option";
  $("#packageForm").innerHTML=`
    <input id="packageTypeId" type="hidden" value="${sid||""}">
    <label>Option / package name<input id="packageName" value="${escapeHtml(p?.name||"")}" required></label>
    <div class="form-grid two">
    ...
    <button class="gold-button full" type="submit">${p?"SAVE OPTION":"ADD OPTION"}</button>
    ${p?`<button class="action-danger full" type="button" data-delete-package="${p.id}">DELETE OPTION</button>`:""}`;
...
function bindEvents(){
  ...
  document.addEventListener("change",e=>{const q=e.target.closest("[data-quick-type-photo]");if(q)quickUpdateShootPhoto(q)});

  document.addEventListener("click",async e=>{
    const t=e.target.closest("[data-open-booking]");if(t)return openBookingModal(t.dataset.openBooking);
    const delBooking=e.target.closest("[data-delete-booking]");if(delBooking)return deleteBookingPermanently(delBooking.dataset.deleteBooking);
    ...
    const cra=e.target.closest("[data-client-request-action]");if(cra)return resolveClientRequest(cra.dataset.id,cra.dataset.clientRequestAction);
...
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("sw-v5.js").catch(console.warn));
bindEvents();
restoreSession();
