...
function clientNameFromBooking(booking) {
  return clientProfile(booking.client_id)?.full_name || "Client";
}

function clientPhoneFromBooking(booking) {
  return clientProfile(booking.client_id)?.phone || "—";
}

function clientEmailFromBooking(booking) {
  return clientProfile(booking.client_id)?.email || "—";
}

function clientInstagramFromBooking(booking) {
  return clientProfile(booking.client_id)?.instagram_handle || booking.instagram_handle || "—";
}
...
function showApp() {
  ...
  $("#ownerName").textContent = "Spaceboi";
  $("#heroOwnerName").textContent = "Spaceboi.";
  $(".hero-card h2").childNodes[0].textContent = `${greetingForNow()}, `;
...
async function refreshAll() {
  try {
    // Load clients first because booking/payment cards use client names.
    await loadClients();
    ...
    console.error("SBS HQ refresh failed:", error);
...
async function loadClients() {
  const { data, error } = await sb
    .from("profiles")
    .select("id,full_name,email,phone,instagram_handle,role,created_at")
    .order("created_at",{ascending:false});
...
function paymentCard(payment, compact = false) {
  ...
  const client = clientProfile(b.client_id)?.full_name || "Client";
...
function renderClients(search = "") {
  ...
  // Show real clients plus any profile that has a booking.
  // This also makes your current test booking display correctly even though
  // it was created with the temporary admin account.
  const clients = state.clients.filter(c => {
    const hasBooking = state.bookings.some(b => b.client_id === c.id);
    if (c.role !== "client" && !hasBooking) return false;
  ...
  $("#clientsList").innerHTML = clients.length ? clients.map(client => {
    ...
    return `
      <article class="client-card">
        ...
        <p class="client-contact-line">${escapeHtml(client.email || "No email")}</p>
        <p class="client-contact-line">${escapeHtml(client.phone || "No phone")} ${client.instagram_handle ? `• ${escapeHtml(client.instagram_handle)}` : ""}</p>
        <div class="card-meta">
...
function openBookingModal(id) {
  ...
  $("#bookingModalContent").innerHTML = `
    <div class="modal-detail-grid">
      ...
      <div class="modal-detail"><span>EMAIL</span><strong>${escapeHtml(clientEmailFromBooking(b))}</strong></div>
      <div class="modal-detail"><span>PHONE</span><strong>${escapeHtml(clientPhoneFromBooking(b))}</strong></div>
      <div class="modal-detail"><span>INSTAGRAM</span><strong>${escapeHtml(clientInstagramFromBooking(b))}</strong></div>
...
async function openPaymentModal(id) {
  ...
  const b = p.bookings || {};
  const paymentClient = clientProfile(b.client_id);
  $("#modalPaymentTitle").textContent = paymentClient?.full_name || "Client";
  ...
  $("#paymentModalContent").innerHTML = `
    <div class="modal-detail-grid">
      ...
      <div class="modal-detail"><span>CLIENT EMAIL</span><strong>${escapeHtml(paymentClient?.email || "—")}</strong></div>
      <div class="modal-detail"><span>CLIENT PHONE</span><strong>${escapeHtml(paymentClient?.phone || "—")}</strong></div>
      <div class="modal-detail"><span>DATE</span><strong>${escapeHtml(dateText(b.preferred_date))}</strong></div>
